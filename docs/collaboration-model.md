# idea-engine — collaboration model
...
- Staleness review cadence: Each wake grooms one section: index must match the folder, built ideas re-badged historical with the merged PR cited, stale ideas parked with a reason. Sections re-derived against the fleet manifest at every groom pass.
