...
**Born-red HOLD.** This card lands first with `Status: in-progress` to hold the substrate gate red on this PR while the verifier, proposal doc, and outbox block are assembled; the flip to `complete` is the final commit and releases the landing workflow.
...
- **G1 — work reduction (tight Dirac herd, all N fail at t=0):** full-jitter mean total attempts is ≥3σ below the deterministic no-jitter total, and the reduction fraction 1 − mean_full/total_none ≥ 0.30. (No-jitter's drain is deterministic in this regime — an exact baseline — so the z uses the full-jitter replication SE.)
- **G2 — thundering-herd flattening (same regime):** the repeated post-herd peak contention (max attempts in any slot after the unavoidable t=0 herd) is ≥3σ smaller under full jitter than the deterministic no-jitter value (N−K); jitter stops the herd re-forming at full strength.
- **G3 — robustness under a shifted arrival distribution (≥3σ, paired):** replacing the synchronized herd with an over-dispersed geometric arrival smear, full jitter STILL reduces total work vs no-jitter on the same arrivals; the paired diff (none − full) is ≥3σ positive across trials.
...
**3.** Is the no-jitter baseline a fair comparator, not a strawman? Both strategies use the identical capped-exponential backoff window; they differ ONLY in whether the wait is the whole window or a uniform draw within it — the single variable the head is about.

**4.** Is the significance valid given the no-jitter arm is deterministic? In the Dirac regime the no-jitter drain is an exact constant (timing is independent of which clients succeed), so it is a fixed baseline and the z is taken against the full-jitter replication SE; G3 uses a proper paired diff where both arms vary.
