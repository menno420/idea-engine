...
- [`write-contract-rate-tier-degeneracy-2026-07-15.md`](write-contract-rate-tier-degeneracy-2026-07-15.md) — sim-ready · process (→ sim-lab): the WRITE contract's two-tier rate limit ships two constants with IDENTICAL average rates ("**Burst**: 10 actions per 10 seconds. / **Sustained**: 60 actions per minute.", `docs/mining-write-contract.md:164-165` @ `b9ade33` — B·T = S·w = 600 exactly) with the window discipline unstated, while the committed reference implementation carries exactly ONE sliding-window knob (`tests/shim/shim_bot.py:115, 340-366`) and the real executor is unbuilt — three exact theorems, every numeral RAN at drafting (V084 lesson): the sustained tier is DEAD INK under every uniform discipline (contacts 60 = S sliding, 60 = S aligned-fixed, bucket min level exactly 50), the burst tier alone over-admits 7/6 (fixed, worst 60 s) / 23/20 (bucket) with a 20 = 2B boundary straddle, and the committed pair sits exactly ON the redundancy boundary (S = 50 flips it live; w = 7 kills it by divisibility) — pre-registered hermetic sim (exact seedless Arms A/B + reporting-only seeded click streams, seeds 20261650–653), REJECT-first; harvested 2026-07-15 for the ORDER 003 fleet-backlogs rotation slot, round 15 opener (PROPOSAL 073)
