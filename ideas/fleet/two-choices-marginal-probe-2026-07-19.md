# PROPOSAL 177 — the second probe does all the work: in randomized dispatch, one extra choice (d=1→d=2) collapses max load exponentially, and that single probe removes more max load than every further probe combined
...
> **Verifier (firsthand):** `ideas/fleet/two_choices_marginal_probe.py` — stdlib-only, SEED=20260717, deterministic double-run; results-dict sha256 625b38309d4e6c8209a74f9123b23d56d769beb4ad592b917093d8c67d234c7f
...
The value of probes is almost entirely front-loaded onto the SECOND probe. With one random choice the maximum load is Θ(log n / log log n); with two it drops to Θ(log log n) + O(1) — an exponential collapse. Because d then enters only inside a log-log term, every probe past the second buys only a shrinking constant. The honest, robust statement is aggregate: the single second probe removes several times more maximum load than every further probe (third, fourth, …) COMBINED — ~4× in dry-sim. Operationally: JSQ(2) captures essentially the whole balancing win; paying for JSQ(3+) — extra fan-out RPCs, tail-latency probes, load queries — is near-worthless spend.
...
| G2 | second-probe dominance (the head) | dom = second_gain − further_gain; z(dom); ratio second_gain/further_gain | dom > 0, z ≥ 3, ratio ≥ 3 |
| G3 | robustness under shifted load (m = n/2) | dom and ratio recomputed in regime B | dom > 0, z ≥ 3, ratio ≥ 3 |
...
```
{
  ...
  "gates": {
    ...
    "g2_second_probe_dominance": {
      "all_further_gain": 0.884,
      "dom_stat": 2.652,
      "pass": true,
      "ratio_second_over_further": 4.0,
      "second_probe_gain": 3.536,
      "z": 46.693563
    },
    "g3_robust_shifted_load": {
      "all_further_gain": 0.992,
      "dom_stat": 3.672,
      "pass": true,
      "ratio_second_over_further": 4.701613,
      "second_probe_gain": 4.664,
      "z": 52.81092
    }
  },
  "params": {
  ...
  "regime_a_load1": {
    "all_further_gain_2_to_dmax": 0.884,
    "gap_1_2": 3.536,
    "gap_2_3": 0.068,
    "gap_3_4": 0.816,
    "maxload_mean": {
  ...
  "regime_b_load2": {
    "all_further_gain_2_to_dmax": 0.992,
    "bins": 4000,
    "maxload_mean": {
      "1": 8.748,
      "2": 4.084,
...
Results-JSON sha256: 625b38309d4e6c8209a74f9123b23d56d769beb4ad592b917093d8c67d234c7f
G1 second-probe jump:      PASS
G2 second-probe dominance: PASS
G3 robust (shifted load):  PASS
```
Both runs (in-process double-call and a second process invocation) produced byte-identical output; results-dict sha256 = 625b38309d4e6c8209a74f9123b23d56d769beb4ad592b917093d8c67d234c7f.

## Reproduce
```
python3 ideas/fleet/two_choices_marginal_probe.py
```
Exit 0 ⇔ all_pass; the printed "Results-JSON sha256" must equal 625b38309d4e6c8209a74f9123b23d56d769beb4ad592b917093d8c67d234c7f.
...
Balls-into-bins models stateless randomized dispatch with instantaneous placement and no departures within the placement window (a heavy-traffic snapshot). It abstracts away service-time correlation and stale load reads; the qualitative doubly-diminishing curve is robust to those (documented for the supermarket/JSQ(d) queueing model too), but exact constants shift with the departure process. Caveat disclosed: at finite n the individual per-probe gaps past d=2 are integer-granular and NON-monotone (in dry-sim d=2→3 ≈ 0 bins while d=3→4 ≈ 0.8 bin); the head is therefore stated and gated as an aggregate — one second probe vs all further probes combined — not as a strictly shrinking per-step sequence.

## Probe report (v0, 2026-07-19)
**1. Is d=2 vs d=1 the only big jump, or do the gaps diminish smoothly?** The second probe (d=1→d=2) removes ~4× more max load than probes 3–4 combined (G2/G3 ratio ≥ 3, z ≫ 10). Individual later gaps are integer-granular and non-monotone at finite n (d=2→3 ≈ 0 bins, d=3→4 ≈ 0.8 bin), so the honest claim is aggregate — the one second probe dominates all further probes combined — not that each successive gap strictly shrinks.

**2. Could the effect be an artifact of n = m (load factor 1)?** No — G3 reruns at m = n/2 (load factor 2) and the dominance persists at ≥3σ with ratio ≥ 3, so it is not tied to exactly-critical loading.
...
**4. Is the max-load statistic too noisy to gate at 3σ?** Over R=250 trials the SE of mean max load is a few hundredths of a bin; the dom statistic clears 3σ by a wide margin in dry-sim (z ≫ 10), so the gate is not marginal.
...
**8. What would falsify the head?** A regime where the second probe's gain is under 3× the combined further-probe gain (ratio < 3), or where d=2 fails to beat d=1 at ≥3σ. The verifier checks exactly these; either failing flips all_pass to false and the proposal is not sim-ready.
