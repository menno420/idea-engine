...
Theta(log n / log log n) to Theta(log log n), but every probe past the second
buys only a vanishing constant -- so the whole dispatch win is the ONE extra
probe; JSQ(3+) barely beats JSQ(2).
...
1->2 gap dwarfs the 2->3 gap by an order of magnitude and 3,4,... sit within
noise of 2, because d enters only inside a log-log term.

Pre-registered gates (>=3 sigma, each tests the head):
  G1  d=2 strictly beats d=1: z(m1 - m2) >= 3, gap > 0.
  G2  second-probe dominance: dom = gap(1,2) - gap(2,3); z(dom) >= 3 AND
      ratio gap(1,2)/gap(2,3) >= 3.
  G3  robustness under shifted load (m = n/2 bins, load factor 2): dom and
      ratio recomputed in that regime; z(dom) >= 3 AND ratio >= 3.
...
def run():
    ...
    dom = g12 - g23
    dom_se = math.sqrt(sA[1] ** 2 + 4 * sA[2] ** 2 + sA[3] ** 2)

    g12B, s12B = _gap(mB, sB, 1, 2)
    g23B, s23B = _gap(mB, sB, 2, 3)
    domB = g12B - g23B
    domB_se = math.sqrt(sB[1] ** 2 + 4 * sB[2] ** 2 + sB[3] ** 2)

    z_g1 = _z(g12, s12)
    z_g2 = _z(dom, dom_se)
    ratio_g2 = g12 / g23 if g23 > 0 else float("inf")
    z_g3 = _z(domB, domB_se)
    ratio_g3 = g12B / g23B if g23B > 0 else float("inf")

    g1_pass = (g12 > 0) and (z_g1 >= SIGMA_GATE)
    g2_pass = (z_g2 >= SIGMA_GATE) and (ratio_g2 >= RATIO_MIN)
    g3_pass = (z_g3 >= SIGMA_GATE) and (ratio_g3 >= RATIO_MIN)
    ...
    results = {
        ...
        "gates": {
            ...
            "g2_second_probe_dominance": {
                ...
                "ratio_g12_over_g23": r6(ratio_g2), "pass": g2_pass,
            },
            "g3_robust_shifted_load": {
                "dom_stat": r6(domB), "z": r6(z_g3),
                "ratio_g12_over_g23": r6(ratio_g3), "pass": g3_pass,
            },
        },
        "all_pass": all_pass,
    }
