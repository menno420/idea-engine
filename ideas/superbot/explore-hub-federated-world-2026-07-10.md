...
One line: the four owner-reserved questions stay with the owner — no sim should settle them — but plan PR 2's progression-balance gate is exactly the kind of input evidence can serve: sweep global-trickle ratios and global-skill effect sizes against the shipped mining/fishing earn shapes (plus PROPOSAL 003's `GAME_ENCOUNTERS` source) so the owner planning session ratifies an evidence-backed default instead of guessing one. (The risk lens returned needs-more-grooming on doc staleness; that ask is consumed by this report's reality-check section — state advances forward-only.)
