...
> **Status:** `binding`
>
> Local copy for idea-engine. The kit-owned work-claim convention
...
## idea-engine specifics (HOST-OWNED section — migrated from root `claims/`, kit upgrade v1.8.0)

This directory REPLACES the repo's original hand-made root `claims/` (the
PR #1 card recipe, seeded pre-kit-convention) — migrated wholesale when the
v1.8.0 upgrade planted the kit-native convention here; `claims-legacy-location`
made root `claims/` a nudged legacy home, and one convention beats two (the
parser-copy-drift lesson applied to claim dirs). Repo-specific rules carried
over unchanged:

- **Section claims**: before working an `ideas/<section>/` tree, claim the
  section here — parallel workers claim **disjoint sections**; a claimed
